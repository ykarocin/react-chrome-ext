...
import { filterDuplicatedDependencies, updateLocationFromStackTrace,filterCFSubStack } from "./dependencies";
...
import { updateGraph } from "./Graph/graphInfos";
...
export default function DependencyView({ owner, repository, pull_number }: DependencyViewProps) {
  ...
  const changeActiveConflict = (dep: dependency) => {
    ...
    if (dep.type.startsWith("CONFLUENCE") && cfLine) {
      ...
      updateGraph(modifiedLines, setGraphData, dep, L, R, CF);
    } else {
      let L: lineData = {
        file: fileFrom,
        line: lineFrom.location.line,
      ...
      updateGraph(modifiedLines, setGraphData, dep, L, R);
    }
  };
  ...
  useEffect(() => {
    const fetchAnalysis = () => {
      getAnalysisOutput(owner, repository, pull_number).then((response) => {
        ...
        if (dependencies.some((dep) => dep.type.startsWith("CONFLUENCE"))){
